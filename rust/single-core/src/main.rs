...
fn main() -> ! {
    ...
    unsafe {
        init_mtvec();
        // Enable machine fast software interrupts
        mie::set_mfast_16();
        // SET machine fast software interrupt in ITC PULP IRQ MASK register
    ...
    loop {
        unsafe {
            // Extract and check interrupt pending or not for machine fast software interrupt
